...
//---------------------------------------------------------
bool getSpadInfo(VL53L0X *dev, uint8_t *count, bool *type_is_aperture);
void getSequenceStepEnables(VL53L0X *dev,SequenceStepEnables * enables);
void getSequenceStepTimeouts(VL53L0X *dev,SequenceStepEnables const * enables, SequenceStepTimeouts * timeouts);
bool performSingleRefCalibration(VL53L0X *dev,uint8_t vhv_init_byte);
...
void writeReg(VL53L0X *dev, uint8_t reg, uint8_t value) {

  msgBuffer[0] = value; // Assign the value to the buffer.
  i2cStat = HAL_I2C_Mem_Write(&hi2c1, dev->g_i2cAddr | I2C_WRITE, reg, 1, msgBuffer, 1, I2C_TIMEOUT);
}

// Write a 16-bit register
void writeReg16Bit(VL53L0X *dev, uint8_t reg, uint16_t value){

  memcpy(msgBuffer, &value, 2); // Assign the value to the buffer.
  i2cStat = HAL_I2C_Mem_Write(&hi2c1, dev->g_i2cAddr | I2C_WRITE, reg, 1, msgBuffer, 2, I2C_TIMEOUT);
}

// Write a 32-bit register
void writeReg32Bit(VL53L0X *dev, uint8_t reg, uint32_t value){

  memcpy(msgBuffer, &value, 4); // Assign the value to the buffer.
  i2cStat = HAL_I2C_Mem_Write(&hi2c1, dev->g_i2cAddr | I2C_WRITE, reg, 1, msgBuffer, 4, I2C_TIMEOUT);
}

// Read an 8-bit register
uint8_t readReg(VL53L0X *dev, uint8_t reg) {
  uint8_t value;

  i2cStat = HAL_I2C_Mem_Read(&hi2c1, dev->g_i2cAddr | I2C_READ, reg, 1, msgBuffer, 1, I2C_TIMEOUT);
  value = msgBuffer[0];
...
uint16_t readReg16Bit(VL53L0X *dev, uint8_t reg) {
  uint16_t value;

  i2cStat = HAL_I2C_Mem_Read(&hi2c1, dev->g_i2cAddr | I2C_READ, reg, 1, msgBuffer, 2, I2C_TIMEOUT);
...
uint32_t readReg32Bit(VL53L0X *dev, uint8_t reg) {
  uint32_t value;
  i2cStat = HAL_I2C_Mem_Read(&hi2c1, dev->g_i2cAddr | I2C_READ, reg, 1, msgBuffer, 4, I2C_TIMEOUT);
...
void writeMulti(VL53L0X *dev, uint8_t reg, uint8_t const *src, uint8_t count){

  memcpy(msgBuffer, src, 4);
  i2cStat = HAL_I2C_Mem_Write(&hi2c1, dev->g_i2cAddr | I2C_WRITE, reg, 1, msgBuffer, count, I2C_TIMEOUT);
}

// Read an arbitrary number of bytes from the sensor, starting at the given
// register, into the given array
void readMulti(VL53L0X *dev, uint8_t reg, uint8_t * dst, uint8_t count) {

	i2cStat = HAL_I2C_Mem_Read(&hi2c1, dev->g_i2cAddr | I2C_READ, reg, 1, dst, count, I2C_TIMEOUT);
}


// Public Methods //////////////////////////////////////////////////////////////

void setAddress_VL53L0X(VL53L0X *dev, uint8_t new_addr) {
  writeReg(dev, I2C_SLAVE_DEVICE_ADDRESS, (new_addr>>1) & 0x7F );
  dev->g_i2cAddr = new_addr;
}

uint8_t getAddress_VL53L0X(VL53L0X *dev) {
  return dev->g_i2cAddr;
}
...
// mode.
bool initVL53L0X(VL53L0X *dev, bool io_2v8, I2C_HandleTypeDef *handler){
  // VL53L0X_DataInit() begin

  // Handler
  memcpy(&hi2c1, handler, sizeof(*handler));
  ...
  dev->g_stopVariable = readReg(dev, 0x91);
  ...
  dev->g_measTimBudUs = getMeasurementTimingBudget(dev);
  ...
  setMeasurementTimingBudget(dev, dev->g_measTimBudUs);
...
bool setSignalRateLimit(VL53L0X *dev, float limit_Mcps)
{
...
float getSignalRateLimit(VL53L0X *dev)
{
...
bool setMeasurementTimingBudget(VL53L0X *dev, uint32_t budget_us)
{
  ...
  if (enables.final_range)
  {
    ...
    dev->g_measTimBudUs = budget_us; // store for internal reuse
  }
  return true;
}
...
// in us
uint32_t getMeasurementTimingBudget(VL53L0X *dev)
{
  SequenceStepEnables enables;
  SequenceStepTimeouts timeouts;
...
bool setVcselPulsePeriod(VL53L0X *dev, vcselPeriodType type, uint8_t period_pclks)
{
...
uint8_t getVcselPulsePeriod(VL53L0X *dev, vcselPeriodType type)
{
  if (type == VcselPeriodPreRange)
  {
...
void startContinuous(VL53L0X *dev, uint32_t period_ms)
{
  writeReg(dev,0x80, 0x01);
  writeReg(dev,0xFF, 0x01);
  writeReg(dev,0x00, 0x00);
  writeReg(dev,0x91, dev->g_stopVariable);
  writeReg(dev,0x00, 0x01);
  writeReg(dev,0xFF, 0x00);
  writeReg(dev,0x80, 0x00);
...
void stopContinuous(VL53L0X *dev)
{
...
uint16_t readRangeContinuousMillimeters(VL53L0X *dev,  statInfo_t_VL53L0X *extraStats ) {
...
uint8_t checkRangeContinuousMillimeters(VL53L0X *dev,  statInfo_t_VL53L0X *extraStats, uint16_t *_distance){
	  uint8_t tempBuffer[12];
	  uint16_t temp;
	  if ((readReg(dev, RESULT_INTERRUPT_STATUS) & 0x07) == 0){
		  return 0;
	  }
	  if( extraStats == 0 ){
	    // assumptions: Linearity Corrective Gain is 1000 (default);
...
// extraStats provides additional info for this measurment. Set to 0 if not needed.
uint16_t readRangeSingleMillimeters(VL53L0X *dev,  statInfo_t_VL53L0X *extraStats ) {
  writeReg(dev,0x80, 0x01);
  writeReg(dev,0xFF, 0x01);
  writeReg(dev,0x00, 0x00);
  writeReg(dev,0x91, dev->g_stopVariable);
...
// timeoutOccurred()?
bool timeoutOccurred(VL53L0X *dev)
{
  bool tmp = dev->g_isTimeout;
  dev->g_isTimeout = false;
  return tmp;
}

void setTimeout(VL53L0X *dev, uint16_t timeout){
  dev->g_ioTimeout = timeout;
}

uint16_t getTimeout(VL53L0X *dev){
  return dev->g_ioTimeout;
}
